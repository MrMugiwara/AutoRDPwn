...
$noadmin=$args[0] ; if($noadmin -like '-noadmin') { powershell.exe -sta -NoProfile -ExecutionPolicy Bypass -File `"$PSCommandPath`" ; exit } else { if (!([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")) { Bypass-UAC "powershell.exe -sta -NoProfile -ExecutionPolicy Bypass -File `"$PSCommandPath`"" ; exit }}
