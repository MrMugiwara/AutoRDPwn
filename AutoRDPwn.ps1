...
if($Language -in 'Spanish') {
  ...
  $Pwn6  = "Agente de sesión de RDP" }
	...
	Write-Host ; Write-Host "$txt23" -NoNewLine -ForegroundColor Gray
        ...
        $hash = $Host.UI.ReadLine() ; if(!$hash) { Write-Host ; Write-Host $txt6 -ForegroundColor Red ; sleep -milliseconds 2500 }} until ( $hash )
	...
	if(!$credential) { [Console]::SetCursorPosition(0,"$cursortop")  
	Write-Host "$txt25" -NoNewLine -ForegroundColor Gray ; Write-Host "********" } ; $Host.UI.RawUI.ForegroundColor = 'Blue'
