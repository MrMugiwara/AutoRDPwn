...
function Remove-Exclusions { 
	...
	if($othermodule -like '3') { Write-Host "$txt21" -ForegroundColor Green ; sleep -milliseconds 2500 ; Write-Host
        ...
        $Host.UI.RawUI.ForegroundColor = 'Gray' ; if($externalscript -like 'http*') { Invoke-Expression (New-Object Net.WebClient).DownloadString("$externalscript") }
        else { Import-Module $externalscript ; Invoke-Expression $externalscript }
        if($externalfunction = $null){ $null } else { Invoke-Expression $externalfunction } ; Write-Host ; $Host.UI.RawUI.ForegroundColor = 'Green' ; pause ; sleep -milliseconds 2500 }
